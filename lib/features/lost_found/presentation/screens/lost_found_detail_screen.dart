...
import 'lost_found_screen.dart' show LostFoundType;
...
class _LostFoundDetailScreenState extends State<LostFoundDetailScreen> {
          ...
          children: [
            Row(
              children: [
                ...
                _LocalTypeChip(type: widget.item.type),
              ],
            ),
            SizedBox(height: 8.h),
            ...
            Text(
              'Reported ${_formatTime(widget.item.timestamp)}',
              style: TextStyle(fontSize: 11.sp, color: Colors.grey),
            ),
            SizedBox(height: 12.h),
            // Contact information
            if (widget.item.contactName?.isNotEmpty == true ||
                widget.item.contactPhone?.isNotEmpty == true) ...[
              Row(
                children: [
                  const Icon(
                    Icons.contact_phone,
              ...
              if (widget.item.contactName?.isNotEmpty == true)
                Text(
                  'Name: ${widget.item.contactName}',
                  style: AppTextStyles.bodySmall.copyWith(
                    color: Colors.black87,
                    height: 1.35,
                  ),
                ),
              if (widget.item.contactPhone?.isNotEmpty == true)
                Text(
                  'Phone: ${widget.item.contactPhone}',
                  style: AppTextStyles.bodySmall.copyWith(
                    color: Colors.black87,
                    height: 1.35,
                  ),
            ...
            StreamBuilder<DatabaseEvent>(
              stream:
                  FirebaseDatabase.instance
                      .ref(
                        'lost_found/${widget.item.userId}/${widget.item.id}/feedback',
                      )
                      .onValue,
            ...
            StreamBuilder<DatabaseEvent>(
              stream:
                  FirebaseDatabase.instance
                      .ref(
                        'lost_found/${widget.item.userId}/${widget.item.id}/reply',
                      )
                      .onValue,
              builder: (context, snapshot) {
            ...
            Builder(
              builder: (context) {
                ...
                final isOwner =
                    currentUser != null &&
                    currentUser.uid == widget.item.userId;
  ...
  Future<void> _submitFeedback() async {
    ...
    if (user == null || user.uid != widget.item.userId) return;
    setState(() => _submitting = true);
    try {
      await FirebaseDatabase.instance
          .ref('lost_found/${widget.item.userId}/${widget.item.id}/feedback')
          .set({
...
class _LocalTypeChip extends StatelessWidget {
  final LostFoundType type;
  const _LocalTypeChip({required this.type});
